package com.niit.hourlywagescalculator;
...
public class CalculateTotalPay {
    ...
    public double calculateHourlyWages(double basePay, int hoursWorked) {
        if (calculateOnHourlyBasis(hoursWorked) && checkBasePay(basePay)){

            double salary =0, extra=0, salaryafter=0, total=0;
            salary = basePay * hoursWorked;
            extra = hoursWorked - 40;
            salaryafter = basePay * extra;
            total = salary + salaryafter;
            return total;
        }
    }
